...
@section('content')
    ...
    <form action="{{url('stories/update-gallery/'.$value->id)}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="list-images row">
            @if (!empty($value->list_img))    
                @foreach (json_decode($value->list_img) as $key => $img)
                    <div class="box-image col-lg-2 col-6">
                        <input type="hidden" name="list_img[]" value="{{ $img }}">
                        <img src="{{ asset('uploads/stories/'.$value->id.'/'.$img) }}" class="picture-box">
