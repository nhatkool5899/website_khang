...
@section('content')
    ...
    <form action="{{url('collected/update-gallery/'.$value->id)}}" method="post" enctype="multipart/form-data">
        @csrf
            ...
            @if (!empty($value->imgs))    
                @foreach (json_decode($value->imgs) as $key => $img)
                    <div class="box-image col-lg-2 col-6">
                        ...
                        <img src="{{ asset('uploads/bo-suu-tap/'.$value->id.'/'.$img) }}" class="picture-box">
